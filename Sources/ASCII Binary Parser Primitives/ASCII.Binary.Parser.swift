//
//  ASCII.Binary.Parser.swift
//  swift-ascii-parser-primitives
//
//  Parses a binary (base-2) integer from ASCII bytes.
//
...
extension ASCII.Binary {
    /// A parser that consumes one or more ASCII binary digit bytes (0x30–0x31)
    /// and accumulates them into a `FixedWidthInteger`.
    ///
    /// ## Usage
    ///
    /// ```swift
    /// let bits = ASCII.Binary.Parser<Input, UInt8>()
    /// let value = try bits.parse(&input) // e.g. 0b1011 == 11
    ///
    /// // Fixed-width: consume exactly eight binary digits (e.g. one byte).
    /// let octet = ASCII.Binary.Parser<Input, UInt8>(count: .exactly(8))
    ///
    /// // Signed: consume an optional leading '+'/'-' (e.g. -0b101).
    /// let signed = ASCII.Binary.Parser<Input, Int8>(sign: .optional)
    /// ```
    ...
    where Input: Sendable, Input.Element == Byte {
        ...
        ///     which consumes every available binary digit byte — the historical behavior.
        @inlinable
...
extension ASCII.Binary.Parser: Parser.`Protocol` {
    /// The value produced when parsing succeeds.
    public typealias Output = T
    /// The error thrown when parsing fails.
    public typealias Failure = ASCII.Binary.Error
    ...
    public func parse(_ input: inout Input) throws(Failure) -> T {
        ...
        while index < input.endIndex {
            ...
            let byte = input[index]
            guard byte >= 0x30, byte <= 0x31 else {
                break
            }
            ...
            // most-negative value (e.g. `Int8 "-10000000"`: 128 has no `Int8`).
            let (product, mulOverflow) = result.multipliedReportingOverflow(by: 2)
