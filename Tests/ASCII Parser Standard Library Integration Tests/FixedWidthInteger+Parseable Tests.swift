...
struct `Parseable Integer Tests` {
    ...
    func `UInt parses via Parseable`() throws {
        let value = try UInt(ascii: "100".utf8.map(Byte.init))
        #expect(value == 100)
    }

    @Test
    func `Int8 parses via Parseable`() throws {
        let value = try Int8(ascii: "127".utf8.map(Byte.init))
        #expect(value == 127)
    }

    @Test
    func `UInt8 parses via Parseable`() throws {
        let value = try UInt8(ascii: "255".utf8.map(Byte.init))
        #expect(value == 255)
    }

    @Test
    func `Int16 parses via Parseable`() throws {
        let value = try Int16(ascii: "8080".utf8.map(Byte.init))
        #expect(value == 8080)
    }

    @Test
    func `UInt16 parses via Parseable`() throws {
        let value = try UInt16(ascii: "65535".utf8.map(Byte.init))
        #expect(value == 65535)
    }

    @Test
    func `Int32 parses via Parseable`() throws {
        let value = try Int32(ascii: "0".utf8.map(Byte.init))
        #expect(value == 0)
    }

    @Test
    func `UInt32 parses via Parseable`() throws {
        let value = try UInt32(ascii: "1000000".utf8.map(Byte.init))
        #expect(value == 1_000_000)
    }

    @Test
    func `Int64 parses via Parseable`() throws {
        let value = try Int64(ascii: "9223372036854775807".utf8.map(Byte.init))
        #expect(value == Int64.max)
    }

    @Test
    func `UInt64 parses via Parseable`() throws {
        let value = try UInt64(ascii: "18446744073709551615".utf8.map(Byte.init))
